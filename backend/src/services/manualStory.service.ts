...
export type ManualStoryGenerationResult = {
  ...
  promptsPlain: string; // JSON objects separated by exactly one blank line
};

function getManualStoryGeminiKey(): string {
  const key = process.env.GEMINI_API_KEY_MANUALSTORY?.trim() || '';
  if (!key) {
    throw new Error(
      'Missing GEMINI_API_KEY_MANUALSTORY. Add a dedicated Gemini key for Manual Story generation',
    );
  }
  return key;
}
...
export async function generateManualStoryPrompts(args: {
  ...
  const parseModelOutput = (rawText: string) =>
    safeParseJSON<{
  ...
  let parsed: ReturnType<typeof parseModelOutput> | null = null;
  ...
  const promptsByScene: ManualStoryPrompt[] = [];
  ...
  if (Array.isArray(parsedOutput.promptsByScene) && parsedOutput.promptsByScene.length > 0) {
    for (const item of parsedOutput.promptsByScene) {
      if (!item || typeof item.sceneIndex !== 'number' || typeof item.imagePrompt !== 'string') continue;
    ...
    for (const item of parsedOutput.scenes) {
      ...
      const candidatePrompt =
        (typeof item.mainImagePrompt === 'string' && item.mainImagePrompt.trim()) ||
