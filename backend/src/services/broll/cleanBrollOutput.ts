 ...
 */
export function cleanBrollOutput(raw: string): string {
  let text = raw
    .replace(/\r\n/g, '\n')
    .replace(/\*\*/g, '')
    .trim();
  ...
  text = text
    ...
    .filter(
      (line) =>
        !/do you want the next/i.test(line) && !/next 4 scene prompts/i.test(line),
    )
    .join('\n')
    .trim();
  ...
  const normalizedBlocks = cleanedScenes
    ...
    .filter(Boolean);
  const joined =
    normalizedBlocks.length > 0 ? normalizedBlocks.join('\n\n') : cleanedScenes;
