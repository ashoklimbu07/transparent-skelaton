 ...
 */
export async function generateBrollPrompts(
  ...
  if (apiKeys.length === 0) {
    throw new Error(
      'No Gemini API keys configured for B-roll. Please set GEMINI_API_KEY_BROLL or GEMINI_API_KEY_BROLL1..N in .env',
    );
  }
  ...
  console.log(
    `📦 Processing in ${totalBatches} batches of up to ${batchSize} scenes each using ${models.length} parallel Gemini clients (round robin).`,
  );
  ...
  for (let waveStart = 0; waveStart < totalBatches; waveStart += models.length) {
    ...
    for (
      let i = waveStart;
      i < Math.min(waveStart + models.length, totalBatches);
      i++
    ) {
      waveIndices.push(i);
    }
